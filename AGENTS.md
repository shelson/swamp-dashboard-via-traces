...
This is a **POC for a realtime swamp software-factory dashboard**.

- **`dashboard/`** — the dashboard system: a stdlib-only Python OTLP receiver + live D3 state-machine view that combines trace liveness with repo polling. See `dashboard/README.md` for details.
- **`models/@swamp/software-factory/demo-factory.yaml`** — a noop software-factory model used to test and demonstrate the dashboard.
- **`workflows/workflow-demo.yaml`** — a noop workflow used to generate trace traffic.
- **`dashboard/drive.py`** — test harness that walks a work item through the demo factory, auto-satisfying gates.

The swamp models and workflows in this repo exist purely to exercise the dashboard — they don't perform real factory automation.
